...
int bombenemy_x,bombenemy_y,bshow=0; //enemy bomb show after crashing in gameover page
...
void photoload()
{
	BG = iLoadImage("Image\\Other\\BG.jpg");
	copter= iLoadImage("Image\\Other\\copter.png");
	enemycopter1 = iLoadImage("Image\\Other\\enemycopter.png");
	...
	game_Pause=iLoadImage("Image\\Other\\Game pause.jpg");
...
void enemybombcrasheasy()
{
	for (int i = 0; i < 1; i++)
	{
		if(bomb[i].enemybombShow)
		{
			...
			{
				bombenemy_x = bomb[i].enemybomb_x;
				bombenemy_y = bomb[i].enemybomb_y;
				cshow=0;
				bshow=0;
				iShowBMP2(cop_x+100, cop_y+20, "Image\\Blast.bmp",0);
				LIFE-=20;
...
{
	if((cop_x == -200  || cop_x + dcopx == 1360) || (cop_y + dcopy == 780 || cop_y == 0))
	{
		game_pause(1);
		flamex=cop_x+60;
		flamey=cop_y;
...
void leveleasygameoverpage(int n) 
{
	...
	if(bshow==1 && LIFE<=20 )
	{
		iShowImage(bombenemy_x, bombenemy_y, 50 , 15 , enemybombpic);
	}
